...
The curated-publication surface has two deliberate depths. Signed-out visitors see a finite, Admin-curated homepage window. The four topic categories each keep an explicit first featured article; `推荐` is a separately editable eligibility pool with an optional default featured article. Without saved preferences, the recommendation window leads with that optional default feature followed by the four distinct category features, then uses a Shanghai-day-stable shuffle of the remaining editorial pool. With saved preferences, the first article is the best matching category feature and every remaining result, including Admin-selected recommendation entries, must still match at least one selected interest; reading level ranks suitable results without emptying a sparse pool. Its final action is a direct “登录查看更多” handoff into the existing account surface, while the following homepage sections remain in the document flow. Signed-in returning users reach publications only after their direct workbench; within that final homepage section they first see the same editorial window, then may reveal the rest of the inventory directly below it. Publication inventory, category placement, recommendation eligibility and the optional default recommendation feature remain separate editorial concepts.
