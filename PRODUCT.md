...
Guests can read immediately and receive a small daily lookup trial. Login is requested only when that trial is exhausted or at a restricted action such as saving, vocabulary/Anki, private translation, or summary. The public-beta login uses an unverified mainland-China phone identifier, nickname, and password without SMS; new registration and voluntary password changes require 8–72 printable characters with at least one letter and one digit, while existing six-digit numeric passwords remain login-compatible until changed. This is a low-friction temporary identity, not proof that the user owns the phone number, so password recovery remains unavailable. Admin may privately issue a unique single-use invitation code that grants Basic, Plus, or Max for a fixed number of days after redemption; the account shows the granted limits and expiry, returns to Free at expiry, and may then redeem another code. Plans and quotas are configurable product experiments; online payment is not connected.
