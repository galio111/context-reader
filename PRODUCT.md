...
The curated-publication surface has two deliberate depths. Signed-out visitors see a finite homepage window whose target is one featured article plus three complete three-card rows. The four topic categories may each keep an explicit first featured article; `推荐` is a separately editable, authoritative Admin whitelist with an optional default featured article. An article removed from that pool never silently returns from the full published inventory. Saved level and interest preferences reorder only articles that remain in the pool. The featured card may be an explicit editorial exception, but the following three rows never use a text-only card; no-image items move to the expanded library's later, interwoven positions rather than forming one block. Its final action is a direct “登录查看更多” handoff into the existing account surface, while the following homepage sections remain in the document flow. Signed-in returning users reach publications only after their direct workbench; within that final homepage section they first see the same image-safe window, then may reveal the rest of the inventory directly below it. Publication inventory, category placement, recommendation-pool membership and the optional default recommendation feature remain separate editorial concepts.
...
2. Preserve flow: common actions such as returning home, editing text directly on the article, copying content, saving articles, and opening vocabulary should be reachable without layout surprises. A visible “返回首页” action always returns home; browser/system Back alone restores a previous Reader article. Entering edit mode must preserve the exact viewport as well as the article's typography, width, wrapping, paragraph spacing, and one-copy content structure.
