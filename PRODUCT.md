...
5. Keep entry compact even when the homepage is expressive: `/home-v2` is the shipped homepage, while `/` redirects to it. Keep a permanent start-reading action outside the closed cover and place standalone lookup, paste, and URL as equal modes in the first reading workbench. Saved articles remain reachable from the shared Menu, and every logical article appears only once in latest-opened order.
