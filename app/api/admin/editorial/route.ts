...
export async function PATCH(request: Request) {
  ...
  if (body.dailyBudgetCny !== undefined && (typeof body.dailyBudgetCny !== "number" || !Number.isFinite(body.dailyBudgetCny) || body.dailyBudgetCny < 0 || body.dailyBudgetCny > 10)) return NextResponse.json({ error: "每日预算须为 0–10 元。" }, { status: 400 });
  if (body.enabled && !process.env.DEEPSEEK_API_KEY) return NextResponse.json({ error: "尚未配置 DeepSeek，无法开启自动精选。" }, { status: 400 });
  if (body.budgetTrial !== undefined && body.budgetTrial !== null && !parseEditorialBudgetTrial(body.budgetTrial)) return NextResponse.json({ error: "临时预算须指定有效日期和大于 0、不超过 10 元的金额。" }, { status: 400 });
  try {
    await withDiscoveryLease(async () => {
      const previous = await getEditorialConfig();
      await writeDiscoverySetting(EDITORIAL_CONFIG_KEY, { enabled: body.enabled, provider: body.provider, dailyReviewLimit: body.dailyReviewLimit, jevMonthlyBudgetUsd: body.jevMonthlyBudgetUsd, dailyBudgetCny: body.dailyBudgetCny ?? previous.dailyBudgetCny, budgetTrial: body.budgetTrial === undefined ? previous.budgetTrial : parseEditorialBudgetTrial(body.budgetTrial) });
    });
