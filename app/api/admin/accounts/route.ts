...
export async function GET() {
  ...
  return NextResponse.json({ profiles, entitlements, plans, limits, actions, executions }, { headers: { "Cache-Control": "no-store" } });
}
...
export async function PATCH(request: Request) {
  ...
  await accountFetch("admin_audit_logs", { method: "POST", headers: { Prefer: "return=minimal" }, body: JSON.stringify([{ admin_label: "password-admin", action: body.action, target_type: typeof body.userId === "string" ? "user" : "quota", target_id: String(body.userId || `${body.planId}:${body.metricKey}`), after_value: body }]) });
