...
test('provider usage settles reservation, records cached tokens and all pipeline costs',async()=>{const store=fixture();await withEditorialBudget('2026-09-21',1,async()=>{await editorialPaidRequest('classification','deepseek-flash','article',800,0,async()=>Response.json({usage:{prompt_tokens:1000,prompt_cache_hit_tokens:900,prompt_cache_miss_tokens:100,completion_tokens:10}}));let spent=await getEditorialSpend('2026-09-21');assert.equal(spent.reservedMicrocny,0);assert.equal(spent.calls,1);assert.equal(spent.inputTokens,1000);assert.ok(spent.actualMicrocny>0 && spent.actualMicrocny<1000);await assert.rejects(editorialPaidRequest('repair','deepseek-flash','article',800,0,async()=>{throw Error('timeout')}));spent=await getEditorialSpend('2026-09-21');assert.ok(spent.reservedMicrocny>0);assert.equal(spent.calls,2);const report=editorialDailyReport('2026-09-21',[],3,false,spent);assert.match(report.subject,/0\/35/);assert.match(report.text,/结果不明请求预留/);assert.match(report.text,/未入选/);},store)});
