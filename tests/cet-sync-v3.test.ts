...
test('real protocol-2 client and IndexedDB round-trip v3 across two devices without losing v2 or article state',async()=>{
 ...
 try{
  ...
  const done=cetFinalize(cetResume(received,'2026-09-24T00:01:00Z','resume'),paper,'manual_submit',undefined,'2026-09-24T00:01:10Z','final');saveCetActivity(done);await syncAccountData();
  assert.ok(cloud.has(`preferences:cet-finalization:v3:${up.id}:final`));await select(first);await syncAccountData();await syncAccountData();
  const replay=readCetActivities().find(a=>a.id===up.id)!;assert.equal(replay.finalizations.final.elapsedMs,40000);assert.equal(replay.answers[up.questionKeys[0]].value,'A');assert.equal(readCetCommitPackages().length,1);
  ...
  assert.ok(cloud.has(`preferences:cet-finalization:v3:${up.id}:final`));
