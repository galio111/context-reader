...
After the first versioned production cutover, archive the bootstrap checkout's original Compose file and install `legacy-bootstrap-compose.disabled.yml` in its place. Core-service maintenance and every app recovery then resolve the accepted `/opt/context-reader-current` snapshot explicitly; the bootstrap directory cannot recreate an unversioned app.
