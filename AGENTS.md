...
- New public recommendations enter through one `/admin` candidate workflow. Browser-local saved articles, manual paste, and URL import are equal source modes; URL intake preserves meaningful article images and returns cover candidates. The system owns difficulty, audience, topics, reading time, and timeliness classification; the admin reviews the article, summary, and cover instead of manually classifying it. The editor only saves candidates, and publication exists only in the candidate list after a read-only user-view preview. Candidate rows reuse `public_articles` with `published=false`, while classification, cover metadata, and source kind live in `imported_article.recommendation`. A recommendation cover is mandatory for publishing; inline article images remain optional. Recommendation-preview word lookup is intentionally deferred until the developer account and admin entry are unified. The recommendation crawler discovers from the code-reviewed RSS/Atom whitelist, deduplicates against public and candidate rows, imports and classifies at most two additions per run, and never publishes automatically. `/api/admin/article-crawler` provides manual topic/difficulty/target-inventory runs; `/api/cron/recommendations` rotates one topic daily and must fail closed unless `CRON_SECRET` matches.
...
- `/home-v2` is the same-project book-space review route and does not replace `/` until it is approved. Its authoritative state order and motion boundaries live in `docs/home-v2-implementation-contract.md`. `app/home-v2/page.tsx` server-loads public articles and passes them to `HomeClient`; `HomeClient` selects `BookHome` only for the book variant and continues to own every real `ReaderView` transition. The route starts as a closed digital book that the user clicks to open. One fixed book scene then holds the real micro context demo and reading workbench, a scroll-triggered reading-preference insert, and the server-rendered recommendation catalogue. Paste, URL, and standalone lookup are equal right-page tabs; standalone lookup expands the real dictionary inside that scrollable page and must not become a separate book chapter. Image upload remains absent. Do not render a full-width top bar or any `CR` square mark; keep only the floating `Context Reader`, start-reading, and Menu controls. The current strip-based page turn is rejected and must be replaced only after the user answers `docs/book-model-direction-questionnaire.html`; the chosen engine must keep one continuous cover, spine, page block, and back board before and after opening. The original desktop cursor-letter trail is a confirmed brand effect: keep it clearly visible above the closed/open book, softly mask it around selectable article text, explanation panels and forms, pause it for overlays, and unmount it inside `ReaderView`; mobile uses touch ripple only.
