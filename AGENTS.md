...
Every user-visible production release must preserve the reading core as a hard gate, even when the edited feature is unrelated: run the automated critical suite, then complete live signed-in smoke tests for contextual word explanation including sentence translation, standalone dictionary, and full-article translation. Contextual explanation and standalone dictionary must prove the default `deepseek-v4-flash` route; the forced retryable-provider regression must also preserve the explicitly configured `deepseek-v4-pro` to `deepseek-v4-flash` fallback path. Build success alone never satisfies this gate.
