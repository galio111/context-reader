...
Every user-visible production release must preserve the reading core as a hard gate, even when the edited feature is unrelated: run the automated critical suite, then complete live signed-in smoke tests for contextual word explanation including sentence translation, standalone dictionary, and full-article translation. Contextual explanation and standalone dictionary must also pass the forced retryable-provider regression proving automatic `deepseek-v4-pro` to `deepseek-v4-flash` failover. Build success alone never satisfies this gate.
