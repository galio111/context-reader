...
- `/home-v2` is the same-project book-space review route and does not replace `/` until it is approved. Its authoritative state order and motion boundaries live in `docs/home-v2-implementation-contract.md`. `app/home-v2/page.tsx` server-loads public articles and passes them to `HomeClient`; `HomeClient` selects `BookHome` only for the book variant and continues to own every real `ReaderView` transition. The route starts as a closed, thick digital book that the user clicks to open. One fixed book scene then holds the real micro context demo and reading workbench, a click-triggered standalone dictionary page, a scroll-triggered reading-preference insert, and the server-rendered recommendation catalogue. Paste, URL, and standalone lookup are equal workbench entry modes; image upload remains absent. The old independent dictionary/recommendation/closing sections must not return. The original desktop cursor-letter trail is a confirmed brand effect: keep it clearly visible above the closed/open book, softly mask it around selectable article text, explanation panels and forms, pause it for overlays, and unmount it inside `ReaderView`; mobile uses touch ripple only.
