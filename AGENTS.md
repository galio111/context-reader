...
- `/home-v2` is the same-project complete book-space review route and does not replace `/` until it is approved. `app/home-v2/page.tsx` server-loads public articles and passes them to `HomeClient`; `HomeClient` selects `BookHome` only for the book variant and continues to own every real `ReaderView` transition. The route now includes the slower opening cover, desktop spread/mobile stacked pages, real micro context demo, equal paste/URL entry, conditional recent-article continuation, a standalone DeepSeek dictionary, server-rendered level-filtered recommendations with a cover-to-reader transition, a durable Supabase Storage feedback form, the full Menu/vocabulary/account/saved-article surfaces, and a light closing page. Homepage image upload remains intentionally absent. Desktop uses the full colored letter-particle field outside quiet controls; mobile uses touch ripple only, and all homepage particles unmount inside `ReaderView`.
