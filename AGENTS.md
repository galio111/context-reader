...
- `/home-v2` is the same-project book-space preview and does not replace `/` until it is approved. `app/home-v2/page.tsx` still server-loads public articles and passes them to `HomeClient`, while `HomeClient` selects `BookHome` only for the book variant and continues to own every real ReaderView transition. The first slice covers the opening cover, the desktop two-page/mobile two-section workspace, the real micro context demo, paste/URL entry, conditional recent-article continuation, Menu, vocabulary, and page-to-ReaderView expansion. It intentionally omits homepage image upload, standalone dictionary, and the recommendation catalogue until their later slices are implemented.
