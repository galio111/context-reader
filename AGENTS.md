...
- Guests receive separate Shanghai-day pools: 10 article word/phrase lookups, 5 standalone dictionary lookups, 2 pasted-text imports and 2 URL imports. Cache hits, validation failures, work cancelled before completion and provider failures that do not produce a completed response do not consume these pools. A standalone dictionary request that has already completed upstream but returns an incomplete result remains charged because the provider cost was incurred; record it as a failed execution and a non-email warning. Save, vocabulary/Anki, private translation and summary require login; admin-prepublished caches remain public. All four guest allowances are server-configurable in Admin.
