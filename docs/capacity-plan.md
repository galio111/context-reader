...
Deployed and measured on 2026-09-22: release `20260922T074500`, parent `20260922T072500`, source `9aa9b225ac85811e2733fec430bfb373b37d793b`. The specified mixed workload passed a repeat, but strict unrestricted capacity acceptance is **not complete**: simultaneous cold downloads still exceed the threshold, one initial mixed request timed out, and browser verification is unavailable.
...
89 critical regressions, five focused cache/concurrency/catalogue tests, TypeScript, local/server production builds, release contracts and static egress audit passed. Full tests use `node --require ./scripts/editorial-server-hook.cjs --import tsx --test tests/*.test.ts tests/*.test.tsx`: the final cumulative source passes 222/224. Two existing editorial-mail expectations fail (`editorial-repair` old 35-article target, `jev-calibration` old cost wording). Both also failed on the unmodified starting parent. A third pre-existing malformed-provider diagnostic failure was fixed by the merged model-control release. Assertions were not weakened.
...
| Check | Result |
| --- | --- |
| Catalogue parity | All 426 ids/order and reader-visible fields preserved; all ids still server-rendered; detail retains editorial evidence |
...
Authenticated login, save/read and protocol-2 stale-write rejection passed with isolated synthetic accounts; both accounts were then deleted by exact id/name/time predicates. Anonymous sync/Admin access returned 401; authenticated Admin session/accounts/articles/automation reads passed. Legacy homepage redirects preserve the query string. All seven services are healthy, backup restoration verified 16 tables, and the accepted parent rollback image exists. Only app/caddy were recreated. Actual browser interaction checks remain incomplete because the browser connector repeatedly failed.
