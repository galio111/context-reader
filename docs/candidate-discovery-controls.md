...
2026-09-06 的生产审计纠正了两个容易误判的数字：当时后台实际启用的是 16 个网站，不是 17 个；2026-09-05 上海日最终自动新增 34 篇候选。2026-09-06 看到约 17 篇时，定时任务仍在按每五分钟一个网站继续运行，并不代表该日最终只有 17 篇。
...
本轮最终生产版本为 `20260906T174008`，父版本 `20260906T173435`，源码 `7c7216d323ca9ca9bff91c69ef234a133fa59e99`。公网连接身份、首页、指南、匿名 Admin/同步拒绝、Admin 四项验收、最新 PostgreSQL 备份还原和当前/父版本回滚镜像均通过。Inside Climate News 在启用后的第一次定时运行尝试 2 篇并新增 2 篇，封面均已转存为本站 WebP；五个首页栏目也已执行一次真实持久化打乱。
