...
2026-09-05：控制功能已在独立分支实现并通过本地后台检查，生产发布与首次真实入库验收尚待完成。本轮从大陆服务器检查了 53 个网站配置，18 个通过近期更新、两篇正文及图片可读取的技术检查；其中 New Atlas 样本仍有重复图库信息，不计划启用。其余 17 站进入本轮启用核验，其中 3 个是专门的较低难度来源。
...
- 新增 10 项策略/提取测试与既有关键回归通过。生产部署与实际模型分类、图片保存、候选写入的端到端结果须补充后才能称为上线。
