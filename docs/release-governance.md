...
Accepted production `20260919T232800`, parent `20260919T230100`, source `5fa612eb68836158d5605f9c5fc4c703a14dc4e5`. Public connectivity and active symlink match. Authenticated Admin returned 2 succeeded GLM-4.5-Air executions from the real failover acceptance; unauthenticated access remains 401. Both records are acceptance tests, not unexpected customer incidents. All 98 tests, build, contracts and egress checks passed. Isolated Chromium component previews at 390 and 1280 px show no horizontal overflow; fixtures cover success/failure, while live API verifies actual data. Signed-in contextual explanation, dictionary and full-article translation all succeeded with `deepseek-flash`; registration/login/protocol-2 sync passed and the test account was removed. Seven-service health, checksum-verified 16-table backup restore and parent accepted rollback image passed. Only app/caddy were recreated. Evidence: `artifacts/zhipu-admin-release-20260919/acceptance.json`, `tests.log`, `build.log`, and component screenshots.
