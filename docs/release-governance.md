...
Accepted production `20260922T070500`, parent `20260921T083200`, source `44803366cdbe9419c2644ad9e1a71c26bdfd2316`. Public mainland identity and active release agree; only app/caddy were recreated. Accented contextual and dictionary streams returned 200 with completion markers, pronunciation returned audio, and authenticated Admin today-feature accounting plus anonymous account/sync/Admin boundaries passed. Responsive image cache, video range/cache and 20 homepage cover variants passed. Seven-service health, 16-table isolated backup restore and current/parent rollback images passed.

89 critical regressions, two accented-input tests, the stream-finalization test, build, release contracts and egress guard pass. Broad test runs still expose an original-parent diagnostic assertion and two unchanged editorial email assertions; full-suite success is not claimed. Browser connection timed out, so desktop/mobile visual and daily-notice feel remain user-device checks. Daily notice is deduplicated per account per browser, not across devices. Evidence: `artifacts/reader-media-0922-audit/`; full chronology remains in product-journey. Editorial settings and the other active model-control task are unchanged by this release.
