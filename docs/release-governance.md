...
Accepted production release `20260913T224956`, parent `20260913T223438`, source `7c34a2ca93922d80bfe08be17b5d54531d535e36`. Public `/api/connectivity` and the active release symlink match; only app/caddy changed and core-service start timestamps remained unchanged. The production build, 107 unique regression tests, release contracts and egress guard passed. Three independent JSDOM/IndexedDB test clients connected to the real public account/sync APIs, each restoring the disposable account's 46 articles, 965 vocabulary entries, 3,000 explanations and 16,000 tombstones with exact learning-payload comparison and zero uploads. Cold reopen and repeated sync passed; client 3 resumed an intentionally interrupted cache download without repeating learning pages. Measured on this host: client 1: authentication 298 ms, learning records 3536 ms, complete restore 7648 ms; client 2: authentication 316 ms, learning records 3454 ms, complete restore 6503 ms; client 3: authentication 284 ms, learning records 2811 ms, complete restore 5781 ms. These timings include the test database runtime and are not physical-browser performance measurements.

Admin boundaries, seven-service health, SHA-verified backup restore and current/parent rollback images passed. The disposable account was removed afterward. The actual user's protected Chrome backup and a fresh read-only cloud snapshot still match all 46 articles and 965 vocabulary entries; no personal cloud data was overwritten or deleted. Browser-control connectivity was unavailable, so migration in the user's currently open Chrome tab and physical phone/other-computer convergence remain unverified until those pages refresh. Keep the private origin backup; do not ask the user to clear website data.
