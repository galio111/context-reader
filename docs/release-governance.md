...
Accepted production release `20260919T230100`, parent `20260919T223456`, source `f012229a94e660799eb3de0df4286e2e5ca8d6da`. Public connectivity and active snapshot agree. Only app/caddy restarted; the five core services retained their start timestamps. Production builds, 96 regression/failover tests, release contracts and static egress checks passed. A loopback-only instance of the exact accepted image forced primary failures and completed real dictionary/contextual streams through GLM-4.5-Air, each with one succeeded execution and one quota unit; the instance was removed. Public dictionary, guest/session and protected sync/Admin boundaries, authenticated Admin reads, disposable account registration/login/protocol-2 sync (account removed), seven-service health, checksum-verified 16-table backup restore and current/parent accepted images passed. Browser-visible mid-stream interruption behavior was not visually verified. Full chronology and timing evidence are in `docs/product-journey.md`; machine-readable evidence is in `artifacts/zhipu-release-20260919/acceptance.json`.
