...
Accepted release `20260913T235513`, parent `20260913T224956`, source `a4ae4698dff80e51685cffa95d08e041e47560b8`, verified on 2026-09-14. Public identity and active symlink match. The served client contains no routine sync/wait/retry banners and retains the separate local-save failure safeguard. Component tests verify silent interrupted recovery, explicit manual retry, progress forwarding, session-expiry reconciliation and preserved local articles. Ordinary logout is verified to send local scope; production tests use three independent cookie sessions, explicitly remove access cookies to force refresh, and replay the logged-out session's saved refresh cookie. Before the fix all three became unauthenticated; afterward the result is `[true, false, true]`. Surviving sessions can exchange a disposable article through real GET/POST sync and one still refreshes after the other logs out. QA session cookies were recovered privately after a test-harness copy error; no personal-user tokens or records were accessed. The test account and its data were deleted after acceptance.

Production build, 88 critical tests, four focused session/sync tests, release contracts, egress guard, Admin checks, seven-service health, backup restore and current/parent images passed. Existing sessions already revoked by the old global logout require login again. Browser tabs sharing cookies remain one session. HTTP/session and component acceptance do not claim physical-device visual acceptance.
