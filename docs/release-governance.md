...
Accepted production `20260922T074500`, parent `20260922T072500`, source `9aa9b225ac85811e2733fec430bfb373b37d793b`. The cumulative capacity release retains model controls, MiMo availability, accented lookup and media changes. Public identity/symlink, all 426 catalogue records, account save/sync conflict protection, Admin boundaries/reads, seven-service health, 16-table backup restore and the accepted parent rollback image passed. Only app/caddy were recreated. Local/server builds, 89 critical and five new capacity regressions passed; the full suite is 222/224 with two reproduced pre-existing editorial-mail expectation failures. A 120-second mixed-load repeat passed all 170 actions, but its first run had one client timeout, simultaneous cold downloads still fail the latency gate, and browser verification remains unavailable. This is a deployed optimization, not unrestricted 1,000-DAU certification. See capacity-plan.md and product-journey.md for exact results; model-control.md retains model-specific acceptance limits.
