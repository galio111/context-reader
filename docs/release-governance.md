...
Production release `20260919T223456`, parent `20260919T223000`, source `1cd456dc9185518ba095a9d782972ffd8dccddb1`. Verified on 2026-09-19.

Public connectivity reports the exact release/parent and `mainland_internal`; only app/caddy were recreated. The reviewed nine-file delta preserves the accepted dictionary and media changes. 89 critical, four media and one gesture regressions, local/server production builds and protected contracts passed. Public Chromium touch checks at 390×844 verified 211px quarter-height retention, below-quarter dismissal, downward flick dismissal, reopening and contextual follow-up submission with UI fixtures; desktop/mobile day/night screenshots passed inspection. Separate live signed-in explanation, dictionary, full translation and follow-up requests succeeded using the existing configured models. The temporary QA account was deleted. Signed-in sync, unprivileged Admin denial, authenticated recovery Admin reads, seven-service health, a 16-table backup restore and the accepted parent image were verified. Physical-device feel remains user acceptance.
