...
同项目的 `/home-v2` 是连续书本空间评审入口，尚未替换 `/`。当前权威交互契约位于 `docs/home-v2-implementation-contract.md`。首页初始是一本文字正面与无字背面分离、有封面厚度、纸芯和接触阴影的闭合数字书，由用户点击打开。打开后始终只有一个固定书本场景：第一幅桌面双页由左侧真实微型文章和正式 `ExplanationPanel`、右侧阅读工作台组成；粘贴文章、输入 URL、单独查词是三个同级入口；继续阅读仅在确有历史时出现。点击单独查词会翻到同一本书内的深度词典页。原生自由滚动驱动纸页转场到阅读偏好夹页，再进入真实推荐目录；偏好包含国内学习阶段/考试路线、阅读强度和兴趣，不再用一排等级按钮代替。推荐列表仍由 server render 注入，缺封面的公开文章不会进入目录；已发布库存为空时显示明确空状态，不伪造文章。推荐封面仍可全屏展开并进入原有 `ReaderView`。桌面鼠标彩色字母位于书本之上并对正文、解释和表单做遮罩避让；进入 Reader 后书本场景和桌面字母层都会卸载。手机端使用单页书，体验页和工作台页分开，单独查词继续以翻页进入词典，只有轻微触点涟漪。主页图片上传仍移除，默认无声音。
