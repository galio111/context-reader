...
- Supabase 用于公开推荐文章、手机号标识 + PIN 账号（邮箱 OTP 仅保留兼容）、用量计数和跨设备学习数据同步
...
- `/api/auth/*`：手机号 + PIN 注册登录、兼容邮箱 OTP/托管登录会话、当前会话与退出。
...
- 当前公开测试入口采用“中国大陆手机号标识 + 昵称注册 + 6 位 PIN”，不发短信，也不验证手机号归属。服务端把手机号映射为保留的内部 Auth 邮箱，PIN 由 Supabase 按密码哈希保存；内部邮箱绝不能展示给用户。邮箱 OTP 代码只保留兼容，微信登录留到有对应主体、通道和转化证据后再做。
...
- 生产数据库、Vercel/Supabase 环境变量和跨设备同步已经可用。手机号 + PIN 让 SMTP 不再阻塞公开测试，但手机号未验证，忘记 PIN 只能由 `/admin` 生成一次性显示的临时 PIN，当前会话为 7 天。面向普通公众的邮箱登录仍未完成；若以后重新开放邮箱入口，必须先配置自定义 SMTP 与包含 `{{ .Token }}` 的模板并验证非团队邮箱。
