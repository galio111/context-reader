...
同项目的 `/home-v2` 已经是完整书本空间评审入口，但尚未替换 `/`。它包括冷白/浅灰蓝数字封面、首次约 2.18 秒且回访约 1.12 秒的可见开封、桌面双页与手机上下页、左页真实微型文章及正式 `ExplanationPanel`、右页同级粘贴/URL 工作台、仅在确有保存文章时显示的继续阅读、独立深度词典、按小学/初中/高中/四级/六级/考研/雅思/托福筛选的真实推荐目录、推荐封面全屏展开并进入原有 `ReaderView`、私有持久化意见反馈、完整 Menu 和轻量收尾页。独立词典通过 `/api/dictionary` 使用 DeepSeek、并发保护和现有查词用量体系，返回 IPA、义项、用法辨析、搭配、词族、近义词和易错点。推荐列表仍由 server render 注入，缺封面的公开文章不会进入目录；当前没有已发布库存时显示明确空状态，不伪造文章。进入 `ReaderView` 后书本首页和桌面彩色字母层都会卸载，手机端只保留轻微触点涟漪。主页图片上传仍按当前决策移除。
