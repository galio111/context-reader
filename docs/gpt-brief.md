...
这份文档用于把 Context Reader 的项目背景、产品目标、技术现状、用户偏好和重要约束同步给 ChatGPT。GPT 不能自动读取本地代码、Codex 记忆或浏览器缓存；当你想在 GPT 里讨论 Context Reader 的产品设计、功能取舍、交互方案、提示词、架构或排期时，可以直接上传或粘贴这份文档。

如果这份文档和代码、`AGENTS.md`、`README.md` 或 `docs/architecture.md` 冲突，以当前代码和 `AGENTS.md` 为准，并让 Codex 先核对。

## 1. 项目一句话

Context Reader 是一个面向中文母语英语学习者的 Next.js 阅读工具。用户可以粘贴英文文章、导入公开 URL 文章或上传英文图片，保存文章，在阅读时点击单词或划选短语获得中文语境解释，把有价值的词汇保存到本地词汇本，并导出 CSV 或通过 AnkiConnect 导入 Anki。它还支持全文翻译、文章编辑、公开推荐文章、管理员预缓存解释和翻译，以及有限的 PWA 离线能力。
...
1. 打开首页。
2. 粘贴英文文章、导入公开网页 URL、上传英文图片，或打开已保存/推荐文章。
...
- 用户明确喜欢原首页中随桌面鼠标移动产生彩色英文字母的效果。这是已确认的品牌动效，不应擅自弱化或藏到不透明书页下面；它在闭合封面和展开书页上都应清楚可见，但要在文章正文、解释面板和表单附近柔和避让，打开 Menu/弹层时暂停，进入正式 ReaderView 后完全消失。手机端只保留轻微触点涟漪，不持续生成跟手字母。
- 不要全局黑色顶栏。
...
图片 block 可以包含 `src`、`alt`、`ocrText`、`layoutWords`、`layoutError`。首页图片阅读会并行请求文本 OCR 和词框识别；只有成功保存了 `layoutWords` 的图片才支持按图中文字位置点击解释。
...
- `/api/ocr-image`：首页上传图片的文本 OCR。
- `/api/ocr-image-layout`：上传图片、data URL 或远程图片的词框识别。
...
- 翻译缓存存于浏览器 `localStorage`。
- 既有整篇合并缓存，也有可复用的 per-block 缓存。
...
- 圆形重新生成按钮才表示强制重翻整篇；它应重翻所有当前 text blocks，发送全文上下文，并且不复用旧的整篇翻译缓存或 per-block 翻译缓存。
...
- 大词汇本使用动态高度虚拟列表，只挂载视口附近的词条；不要在每个滚动事件里更新 React 状态，也不要让每个虚拟行重复初始化浏览器 TTS。
- 短条目要紧凑，长条目要完整可见。
- 首页词汇本是居中的顶部 dialog。
- 阅读页词汇本是右侧 drawer。
- 两种入口打开词汇本时都要锁定底层页面滚动，关闭后恢复原滚动位置；词汇列表必须保留独立、明确高度的内部滚动区域并隔离 overscroll，首页的全局滚轮教学锁和场景吸附不得拦截词汇本内部的滚轮或触摸事件。
...
同项目的 `/home-v2` 是连续书本空间评审入口，尚未替换 `/`。当前权威交互契约位于 `docs/home-v2-implementation-contract.md`。首页初始是一本文字正面与无字背面分离、有封面厚度、纸芯和接触阴影的闭合数字书，由用户点击打开。界面不再使用 `CR` 方块标，顶部不铺整条栏，只保留悬浮的 `Context Reader`、开始阅读与 Menu；封面主标题也是 `Context Reader`，副标题为“语境翻译魔法书”。打开后始终只有一个固定书本场景：第一幅桌面双页由左侧真实微型文章和正式 `ExplanationPanel`、右侧阅读工作台组成；粘贴文章、输入 URL、单独查词是三个同级 Tab；继续阅读仅在确有历史时出现。单独查词的真实词典直接显示在可滚动右页内，不再翻到独立词典章节。原生自由滚动驱动纸页转场到阅读偏好夹页，再进入真实推荐目录；偏好包含国内学习阶段/考试路线、阅读强度和兴趣，推荐页不再重复显示一整行阶段筛选。推荐列表仍由 server render 注入，缺封面的公开文章不会进入目录；已发布库存为空时显示明确空状态，不伪造文章。当前竖向切片翻页已被明确否决，替换路线等待用户在 `docs/book-model-direction-questionnaire.html` 中选择；新实现必须保持开书前后同一书壳、书脊、纸芯与封底，并让整张纸连续弯曲。推荐封面仍可全屏展开并进入原有 `ReaderView`。桌面鼠标彩色字母位于书本之上并对正文、解释和表单做遮罩避让；进入 Reader 后书本场景和桌面字母层都会卸载。手机端使用单页书，体验页和工作台页分开，单独查词仍位于工作台页，只有轻微触点涟漪。主页图片上传仍移除，默认无声音。

- `context-reader/docs/home-direction-1-workbench.html`
- `context-reader/docs/home-direction-2-command.html`

`docs/visual-metaphors-demo.html` 与 `.codex/visualizations/2026/07/11/home-metaphor-compare/visual-metaphors.html` 是早期视觉隐喻对比实验，不能把其中任何一套完整概念直接当作当前实现。`docs/home-complete-ui-prototype.html` 是生产首页的设计基线；对应 React 实现在 `components/ImmersiveHome.tsx`，由 `ArticleInput` 复用原有导入、推荐、保存文章和生词本回调。实现借鉴设计工作室网站的空间连续性，但不复制参考网站的专有代码或素材。
...
- 本地已保存文章、粘贴文章与输入 URL 是同级入口，共用同一个候选编辑器；URL 导入会保留有效正文图片，并返回可选封面候选。手动 URL 与系统自动候选抓取必须复用同一个 `/api/import-url` 正文边界：按嵌套 DOM 容器剔除导航、引用/来源、相关阅读、推荐、评论、订阅和社交模块，再用块级结束标记兜底，不能把站点尾部杂项写入正文。
- 自动判断适合中国学习者的难度、CEFR 参考、人群、兴趣主题、阅读时间和时效属性；这些结果由系统负责，不要求管理员手动选择类型。
- 编辑器只保存候选，不直接发布；候选列表提供只读的用户视图预览，发布入口只存在于候选列表。
- 候选预览内的查词暂缓到开发者账号与后台入口打通之后再做；当前不应为了预览启用独立的管理员查词额度或副作用。
...
OCR 需要区分首页图片阅读与 URL 导入文章图片，不应笼统描述成全部启用或全部禁用。
