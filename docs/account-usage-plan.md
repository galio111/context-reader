...
## Browser persistence and login completion
...
The accounts section now has four independent views with persistent desktop/mobile navigation. Default reporting is Shanghai today; feature/model/account costs, date/source filters, unknown charges and retained quota details follow [admin-usage-report.md](admin-usage-report.md). Reporting is read-only and does not change the consumption rules above.
