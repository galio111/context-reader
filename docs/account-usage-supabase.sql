...
create table if not exists public.user_entitlements (
  ...
  source text not null default 'signup' check (source in ('signup', 'admin', 'payment', 'promotion', 'invite')),
...
begin
  ...
  if v_code.revoked_at is not null then
    raise exception 'invitation_code_revoked';
  end if;
  if v_code.redeemed_at is not null then
    raise exception 'invitation_code_redeemed';
  end if;
  if v_code.redeem_by is not null and v_code.redeem_by <= v_now then
  ...
  if found
    and v_entitlement.source = 'invite'
    and v_entitlement.ends_at is not null
    and v_entitlement.ends_at > v_now then
    raise exception 'active_invitation_entitlement';
  end if;

  if found
    and v_entitlement.plan_id <> 'free'
    and (v_entitlement.ends_at is null or v_entitlement.ends_at > v_now) then
  ...
  insert into public.user_entitlements (
    user_id, plan_id, source, starts_at, ends_at, updated_at
  ) values (
    p_user_id, v_code.plan_id, 'invite', v_now, v_ends_at, v_now
  )
  ...
  set redeemed_at = v_now,
      ...
      grant_ends_at = v_ends_at
  where id = v_code.id;

  insert into public.admin_audit_logs (
    admin_label, action, target_type, target_id, after_value
  ) values (
    'user-self-service',
    ...
    p_user_id::text,
    jsonb_build_object(
      'invitationId', v_code.id,
      'planId', v_code.plan_id,
      'startsAt', v_now,
      'endsAt', v_ends_at
    )
  );
