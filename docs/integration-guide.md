...
- saving incomplete candidates with `published=false`, including intentional text editions without a cover,
...
The reader uses this stream as the first and normally only generated request on a cache miss. Streamed text fills the final `ExplanationPanel` tree progressively; after the completion marker, the client parses and caches that same visible result and derives durable Anki fields locally. `/api/explain-word` is a sequential fallback only when the stream cannot produce a complete explanation, never a parallel second DeepSeek call. Progressive output, completed display, and cache replay therefore keep identical content, field order, spacing, and wrapping width. A stable scrollbar gutter prevents action controls from changing text width when they appear. Closing or switching away from unfinished work aborts the active request and posts its action id as a bounded same-origin `text/plain` body to `/api/lookup-cancel`, which also stops linked upstream work when the reverse proxy does not surface a disconnect. Timely client cancellation is refunded and creates no Admin report or alert. Genuine persistent transport, HTTP, empty-content or parse failures retain their specific private diagnostic codes.
