...
Sync checkpoints at `context-reader:sync-state:v2` use backward-compatible LZ UTF-16 compression. Legacy JSON is compressed before incoming payloads are merged, and the in-memory checkpoint advances only after durable storage succeeds. Login, manual sync and the one-time `context-reader:sync-storage-recovery:20260913` repair replay the bounded protocol-2 active/deleted snapshot even with a current cursor. Full reconciliation compares every stored article and vocabulary id and normalized JSON payload with the snapshot plus acknowledged CAS writes before reporting verified counts or setting the repair marker. Missing local records never imply deletion; existing conflict and tombstone rules remain intact. Ordinary background polls retain incremental, idle-deferred behavior. Malformed or incomplete acknowledgements cannot report completion. Storage exhaustion is presented as a storage fault; authentication remains valid and the login dialog retries sync without resubmitting credentials.
