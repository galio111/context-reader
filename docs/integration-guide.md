...
For accounts and usage, also set an independent `ACCOUNT_COOKIE_SECRET` and run `docs/account-usage-supabase.sql`. The visible beta flow uses `/api/auth/phone-register` and `/api/auth/phone-login`: the server maps a mainland-China phone identifier to a reserved internal Auth email, marks it unverified, and requires new passwords to be 8–72 printable ASCII characters with at least one letter and one digit. Existing six-digit numeric passwords remain accepted for legacy login until the user changes them. It sends no SMS, exposes no password recovery, and the internal email must never be displayed. Legacy email OTP remains available in code; if exposed publicly later, configure custom SMTP and make the template include `{{ .Token }}`. The service-role key is server-only. Do not create a `NEXT_PUBLIC_SUPABASE_SERVICE_ROLE_KEY`. Optional cost-rate overrides are `DEEPSEEK_CACHE_HIT_USD_PER_MILLION`, `DEEPSEEK_CACHE_MISS_USD_PER_MILLION`, and `DEEPSEEK_OUTPUT_USD_PER_MILLION`.
...
`GET /api/public-articles` lists public articles. `GET /api/public-articles/[id]` returns one article with preloaded word explanations and full-article translation caches. `GET/POST/PATCH/DELETE /api/admin/article-candidates` manages the review queue, `POST /api/admin/article-classification` classifies content, `POST /api/admin/article-covers` uploads cover files, and its protected `PATCH` action localizes selected or all legacy published external covers and body images. Remote candidate covers and body images are fetched at publication through the pinned-DNS safe fetcher, normalized to bounded WebP files with Sharp, and stored under content-addressed keys in the active public Storage bucket; a body-image localization failure blocks publication instead of shipping an external hotlink. Because `static.time.com` is unreliable from the mainland host, TIME assets may use `images.weserv.nl` only as a server-side ingestion fallback; downloaded bytes still pass the same type, size, pixel and Sharp checks, and stored article data receives only the resulting first-party Storage URL. `ops/mainland/repair-public-covers.py --id ARTICLE_ID` provides the server maintenance path without exposing credentials. `GET/PATCH/POST /api/admin/article-crawler` exposes the crawler state, saves the daily enabled/time/count controls, starts manual runs, and sends an explicit test email. Manual crawler requests send `topic`, `difficulty`, `maxNewArticles` from one to six, and `inventoryScope`; they intentionally add up to that many new candidates regardless of existing inventory while preserving URL deduplication and all import/classification checks. Admin writes require the admin session cookie.
