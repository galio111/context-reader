...
Returns an `article` object with `title`, `url`, `siteName`, plain `text`, structured `blocks`, and optional `byline`, `publishedTime`, and `language`, plus `metadata.description` and up to twelve `metadata.coverCandidates`. Cover candidates prefer Open Graph and Twitter metadata, then meaningful article images. Remote scripts are never executed. After DOM extraction, only selected meaningful image blocks are fetched, validated, converted to first-party WebP assets and rewritten before the response. A block that cannot be stored is omitted and reported in `mediaNotice`; intact English text still returns and the failed import does not leave a browser external image URL.
