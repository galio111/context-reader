...
For accounts and usage, also set an independent `ACCOUNT_COOKIE_SECRET`, run `docs/account-usage-supabase.sql`, and enable email auth in Supabase. The app accepts both Supabase's default email login link and a six-digit OTP. Before public launch, configure custom SMTP and make the magic-link template include `{{ .Token }}`; the free-tier default sender only delivers to project-team addresses and does not allow template customization. The service-role key is server-only. Do not create a `NEXT_PUBLIC_SUPABASE_SERVICE_ROLE_KEY`. Optional cost-rate overrides are `DEEPSEEK_CACHE_HIT_USD_PER_MILLION`, `DEEPSEEK_CACHE_MISS_USD_PER_MILLION`, and `DEEPSEEK_OUTPUT_USD_PER_MILLION`.
...
The browser talks only to Context Reader server routes; it never receives the Supabase service-role key or a public Supabase client key. `/api/auth/request-otp` and `/api/auth/verify-otp` provide six-digit email login, while `/api/auth/adopt-session` accepts the hosted Supabase login result. `/api/auth/session` reads or refreshes the server-managed account session and `/api/auth/logout` requires a successful final sync before clearing account-associated local data.
