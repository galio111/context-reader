...
- Review the complete same-project book-space direction at `/home-v2` without replacing the production homepage. It uses a slower digital-cover opening, a desktop spread or stacked mobile pages, the real explanation panel and word/phrase routes, equal paste and URL import, conditional “continue reading,” a standalone DeepSeek dictionary, level-filtered server-rendered recommendations whose cover expands into the same `ReaderView`, durable private feedback storage, a complete Menu, and a light closing page. Homepage image upload remains intentionally absent; desktop letter particles stop around quiet controls and disappear entirely inside the reader, while mobile uses touch ripple only.
