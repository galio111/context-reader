...
Candidate discovery is controlled per website in Admin: daily targets, source verification, readable-image/content gates and quick rejection feedback. The editorial workbench shuffles older published selections across every homepage category while preserving today's selections at the front. Current source evidence and rollout limits are documented in [Candidate discovery controls](docs/candidate-discovery-controls.md).

GitHub default `main` carries the cumulative accepted source and reconciled documents. After validated work, both the task branch and default branch must be pushed and their remote commit verified; a branch-only push is incomplete. The live release identity and evidence are recorded in [release governance](docs/release-governance.md).
...
- Open the homepage dictionary as a draggable, resizable desktop window or a bottom-docked mobile sheet; results scroll independently and desktop geometry is not persisted across sessions.
...
- Public beta registration uses nickname + mainland-China phone identifier + a password of 8–72 non-space printable ASCII characters containing at least one letter and one digit; existing six-digit numeric passwords remain login-compatible until voluntarily changed. No SMS or phone-ownership verification is provided.
...
- Registration collects only nickname, phone identifier and password. Publication personalization owns level/interests; legacy demographic fields are not a new registration step.
...
- Online payment is not connected. Commercial labels remain behind their build-time flag; actual usage balances and reset times are visible to every signed-in account.
...
- `/admin` accepts saved, pasted, URL-imported and crawler-discovered candidates. Its daily editorial workbench combines the real Reader, autosaved metadata, previous/next navigation, expandable candidate/published drawers and one-step URL intake. On desktop the inspector stays left while lookup, vocabulary, saved articles and queue controls stay right; on mobile the top row is `返回后台 / 文章设置 / 精选 / 候选 / Menu`, and each queue opens as a top-down list. The crawler uses reviewed RSS/Atom sources, requires more than 400 English words plus a usable image for every automatic candidate, records the source publication time when available, and keeps manual single-source intake as candidates. The opt-in [automatic editorial pipeline](docs/automated-editorial.md) reviews full text and actual images before publishing, targeting 30 qualified articles per Shanghai day.
...
首页与精选外刊并列的真实阅读题库，复用 ReaderView；支持考试、边读边做、按卷/题型、手动计时、进度同步与重做。题库扩展至 2022–2026 年来源中有完整阅读题序及答案解析的 53 套试卷（四级 25 套、六级 28 套），材料清单、性能与扩库流程见 [docs/cet-reading.md](docs/cet-reading.md)。本轮生产状态以 release-governance 和产品历程为准。
