...
- Review the same-project continuous book-space direction at `/home-v2` without replacing the production homepage. It begins as a closed digital hardback that the user clicks to open. A single fixed book scene then carries the real explanation demo and reading workbench, a page-turn dictionary branch, a reading-preference insert, and server-rendered recommendations whose cover expands into the same `ReaderView`. Paste, URL import, and standalone lookup are equal workbench choices; “continue reading” appears only when history exists. Homepage image upload remains absent. The original desktop cursor-letter trail stays visibly above the book while masking itself around reading text, explanations and forms, then disappears inside the reader; mobile uses a single-page book, touch ripple, and separate demo/workbench pages.
