...
- `/admin` accepts saved, pasted, URL-imported and crawler-discovered candidates. Its daily editorial workbench combines the real Reader, autosaved metadata, previous/next navigation, expandable candidate/published drawers and one-step URL intake. On desktop the inspector stays left while lookup, vocabulary, saved articles and queue controls stay right; on mobile the top row is `返回后台 / 文章设置 / 精选 / 候选 / Menu`, and each queue opens as a top-down list. The crawler uses reviewed RSS/Atom sources, requires more than 400 English words plus a usable image for every automatic candidate, records the source publication time when available, and never auto-publishes.
