...
- Review the same-project book-space first slice at `/home-v2` without replacing the production homepage. It auto-opens a digital cover, renders a desktop spread or two stacked mobile pages, reuses the real explanation panel and word/phrase routes in its micro article, keeps paste and URL import equal, hides “continue reading” when no saved article exists, and expands the selected page into the same `ReaderView`. Image upload, the standalone dictionary, and the recommendation catalogue are intentionally outside this first slice.
