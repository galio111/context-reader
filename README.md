...
- Review the same-project continuous book-space direction at `/home-v2` without replacing the production homepage. It begins as a user-opened digital hardback and keeps one fixed book scene for the real explanation demo, reading workbench, preference insert, and server-rendered recommendations. Paste, URL import, and standalone lookup are equal right-page tabs; the real dictionary expands inside that scrollable page instead of opening a separate book chapter. “Continue reading” appears only when history exists, and homepage image upload remains absent. The page-turn engine is awaiting the user's choice in `docs/book-model-direction-questionnaire.html`; the accepted replacement must preserve one continuous cover/spine/page block and use a whole bending page rather than vertical strips. The original desktop cursor-letter trail stays visibly above the book while masking itself around reading text, explanations and forms, then disappears inside the reader; mobile uses a single-page book, touch ripple, and separate demo/workbench pages.
...
Open `/admin`, enter `ADMIN_PASSWORD`, then build a candidate from a browser-local saved article, pasted text, or a public URL. All three sources use the same editor. Classification is system-owned and read-only; the admin checks the article, generated summary, and cover, then saves to the candidate list. URL intake keeps meaningful inline images and proposes available article images as covers. Candidate rows provide a read-only user-view preview; word lookup in that preview is deferred until the developer account and admin entry are unified. Publication exists only in the candidate list. Automatic discovery is collapsed below the main workflow and accepts a topic, optional difficulty, and target inventory; it reads the reviewed NASA, ScienceDaily, Smithsonian Magazine, Aeon, Literary Hub, and NPR Technology RSS/Atom feeds, deduplicates source URLs, imports the full article, and saves at most two new candidates per run. A daily Vercel Cron run rotates through the six topics and targets six reviewed-or-pending articles per topic. Neither manual nor scheduled discovery publishes automatically.
