...
export const FEATURE_SHOWCASE = [
  { id: "publications", label: "发现外刊", detail: "兴趣 × 难度", title: ["世界很大，", "从你想读的开始。"], paragraphs: ["科学的新发现，文化的新视角，商业与生活的另一面。让真实外刊成为你的日常读物。", "选好兴趣与阅读难度，找到既想读、又读得下去的内容。"], color: "#dce9f6", screens: [{ label: "外刊与个性化推荐", src: "/showcase/publications-v2.mp4" }] },
...
function Recording({ src, label, playing, onBuffered }: { src: string; label: string; playing: boolean; onBuffered: () => void }) {
  ...
  return <div className={styles.recording} data-has-video={Boolean(src)}>
    {src && !failed ? <>
      <video ref={video} src={loadedSrc} poster={loadedSrc ? poster : undefined} muted loop playsInline preload={playing ? "auto" : "metadata"} onTimeUpdate={reportPlayback} onProgress={reportBuffered} onLoadedData={reportBuffered} onCanPlayThrough={reportBuffered} onError={() => { if (desktopWebm && loadedSrc === desktopWebm) setLoadedSrc(src); else if (loadedSrc) setFailed(true); }} aria-label={label} />
  ...
  return <section ref={sectionRef} className={styles.showcase} aria-label="功能展示" data-feature={feature.id} data-motion={motionEnabled} data-playing={visible && !paused}>
    ...
    <div className={styles.inner}>
      <div id="feature-showcase-panel" role="tabpanel" aria-labelledby={`feature-tab-${feature.id}`} className={styles.presentation}>
        ...
        <div ref={mediaRef} className={styles.media} style={visible && feature.id === "publications" ? { "--poster": "url(/showcase/publications-v2.webp)" } as CSSProperties : undefined}>
