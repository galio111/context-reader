    ...
    <aside className="h-full min-h-0 overflow-y-auto rounded-[18px] border border-[#e0e0e0] bg-white p-5 overscroll-contain [-webkit-overflow-scrolling:touch]" data-native-selection="blue">
      <header className="flex items-start justify-between gap-3 border-b border-[#e0e0e0] pb-4">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs leading-5 tracking-[-0.12px] text-[#7a7a7a]">按原文段落对齐，优先保持术语、指代和上下文一致。</p>
        </div>
        {hasTranslations && (
      ...
      {!requested && !hasTranslations && !loading && !error && (
        <div className="py-6">
          ...
          <button type="button" className="mt-4 h-10 rounded-full bg-[#0066cc] px-4 text-sm tracking-[-0.224px] text-white transition active:scale-95 disabled:bg-[#d2d2d7]" onClick={onGenerate} disabled={blocks.length === 0}>开始翻译</button>
        </div>
      )}

      {loading && !hasTranslations && (
        <div className="py-6">
          <div className="space-y-3"><div className="h-4 w-5/6 rounded-full bg-[#ececf0]" /><div className="h-4 w-full rounded-full bg-[#ececf0]" /><div className="h-4 w-2/3 rounded-full bg-[#ececf0]" /></div>
      ...
      {error && !loading && (
        <div className="mt-4 rounded-[14px] border border-red-200 bg-red-50 p-3 text-sm leading-6 text-red-700">
          <div className="flex items-start justify-between gap-3"><p className="min-w-0">{error}</p><button type="button" className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-red-300 bg-white text-base leading-none text-red-700 transition hover:bg-red-100 active:scale-95" onClick={onGenerate} aria-label="继续未完成的全文翻译" title="继续未完成的全文翻译">↻</button></div>
        </div>
      )}
        ...
        <div className="space-y-5 py-5">
          ...
          {!loading && !error && hasIncompleteTranslations && (
            <div className="flex items-center justify-between gap-3 rounded-[14px] bg-[#f5f5f7] px-3 py-2 text-xs leading-5 tracking-[-0.12px] text-[#333333]">
              ...
              <button type="button" className="shrink-0 rounded-full bg-[#0066cc] px-3 py-1.5 text-white transition active:scale-95" onClick={onGenerate}>继续翻译</button>
            </div>
          )}
          {hasOutdatedTranslations && !loading && <div className="rounded-[14px] bg-[#fff8e5] px-3 py-2 text-xs leading-5 tracking-[-0.12px] text-[#6b4b00]">{staleBlockIds.length > 0 && removedTranslationCount > 0 ? `有 ${staleBlockIds.length} 段原文已修改，${removedTranslationCount} 段已删除。点击右上角只更新修改部分并同步移除旧译文。` : staleBlockIds.length > 0 ? `有 ${staleBlockIds.length} 段原文已修改。点击右上角只更新修改部分。` : `有 ${removedTranslationCount} 段原文已删除。点击右上角同步移除旧译文。`}</div>}
          {blocks.map((block) => {
            const translation = translationById.get(block.id);
            if (!translation) return null;
            const isHeading = block.type === "heading" || block.type === "subheading";
            return <section key={block.id} className="border-b border-[#f0f0f2] pb-4 last:border-b-0 last:pb-0"><p className="text-xs leading-5 tracking-[-0.12px] text-[#7a7a7a]">{block.text}</p><p className={`mt-2 tracking-[-0.224px] text-[#1d1d1f] ${isHeading ? "text-base font-semibold leading-7" : "text-sm leading-6"}`}>{translation}</p></section>;
          })}
        </div>
      )}
