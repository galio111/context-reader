...
import { EDITORIAL_CATEGORIES, editorialCategoryForArticle, editorialCategoryLabel, type EditorialCategory, type PublishedArticlePlacement } from "@/lib/editorialCuration";
...
interface ClassificationResponse {
  ...
  topics: ArticleTopic[]; homepageCategory: EditorialCategory; wordCount: number; timeliness: ArticleTimeliness; reviewNotes: string;
  ...
  async function handleReclassify() {
    ...
    try {
      ...
      setDraft((current) => ({ ...current,
        ...
        homepageCategory: locked.has("homepageCategory") ? current.homepageCategory : classification.homepageCategory,
  ...
  return (
    ...
    <aside className={styles.inspector} data-open={mobileOpen || undefined} aria-label="文章设置">
      <div className="shrink-0 border-b border-[#d7dde2] bg-[#f7f9fa] px-4 pb-3 pt-4">
        ...
        {articleKind === "published" && error && <p className="mt-2 text-xs font-medium text-[#9a3030]" role="alert">{error}</p>}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-4">
        ...
        <section className="mt-4 border-y border-[#d7dde2] py-3" aria-labelledby="admin-article-summary-heading">
          <h2 id="admin-article-summary-heading" className="text-sm font-semibold text-[#17212b]">文章梗概</h2>
          <label className={`${labelClass} mt-2`}>中文摘要<ClearableField value={draft.summary} onClear={() => setDraft((current) => ({ ...current, summary: "" }))} label="清空中文摘要" disabled={busy} multiline><textarea className={`${inputClass} min-h-28 resize-y leading-6`} value={draft.summary} onChange={(event) => setDraft((current) => ({ ...current, summary: event.target.value }))} maxLength={1000} disabled={busy} /></ClearableField></label>
        </section>
        <div className="mt-4 grid gap-3">
          <label className={labelClass}>首页栏目<select className={inputClass} value={draft.homepageCategory} onChange={(event) => setDraft((current) => ({ ...current, homepageCategory: event.target.value as EditorialCategory }))} disabled={busy}>{EDITORIAL_CATEGORIES.map((item) => <option key={item} value={item}>{editorialCategoryLabel(item)}</option>)}</select></label>
