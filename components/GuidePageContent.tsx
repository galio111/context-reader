...
const quickSteps = [
  ...
  ["留下值得复习的词", "把反复遇到、影响理解或想主动使用的表达加入生词本，需要长期记忆时再导入 Anki。"],
] as const;
...
const featureGroups = [
  ...
  ["把阅读变成长期积累", "生词本保存当时的原句、语境含义和学习补充。登录后可以跨设备继续阅读，也可以把词导入 Anki 复习。", ["语境完整的生词", "跨设备继续阅读", "Anki 间隔复习"], "peach"],
] as const;
...
const faqGroups = [
  ["阅读与查词", [
    ...
    ["一定要注册账号吗？", "不需要。游客可以直接阅读并试用查词与导入；保存文章、生词本、Anki、私有全文翻译和总结需要登录。"],
  ]],
  ["Anki", [
  ...
  return <main className={styles.guide} data-embedded={embedded || undefined}>
    ...
    <div className={styles.shell}>
      ...
      <article className={styles.content}>
        <section id="developer" className={styles.intro} aria-labelledby="guide-title"><p className={styles.introLabel}>为什么做这个网站</p><h1 id="guide-title">开发者的话</h1><div className={styles.developerCopy}><p>我做 Context Reader，是因为自己在读外刊和长文章时，也经常被密集的生词劝退。把词一个个复制到词典里很慢，同一个词又会列出很多意思，读完一篇文章常常要花很久。我希望阅读可以简单一点：直接在原句里看懂这个词此刻表达什么，继续把文章读下去；遇到真正值得积累的词，就连同原句和语境保存下来，再导入我后来找到的 Anki，形成阅读、理解和复习的闭环。</p><p>网站也会持续整理我精选的英文外刊，满足日常阅读需要。你也可以复制自己的文章，或输入网址直接阅读。希望它能让大家少一点查词的消耗，更轻松地学英语，更顺畅地读完真正想读的内容。</p></div></section>
